...
from subscribe_3 import subscribe_3
...
power = False  # Variable to store the state of the button
...
subscribe_3_inst_1 = subscribe_3()

rgb_inst_1.power_on_loop()

while True:
    try:
        decrypted_message = subscribe_3_inst_1.main_loop()
        # print(">>>>>> " + str(decrypted_message))
        if decrypted_message is None:
            continue
        else:
            extracted_part = decrypted_message.split(': ')[1]
            # print(">>>>>> " + extracted_part)
        
        # if button_inst_1.on(X) == 1 or extracted_part == 'turn on':
        if extracted_part == 'turn on':
            # print("balle balle on")
            power = True
        ...
        while power is True:
            ...
            if decrypted_message is None:
                ...
                extracted_part = decrypted_message.split(': ')[1]
                # print(">>>>>> " + extracted_part)
            if extracted_part == 'turn off':
                print("balle balle off")
                power = False
                rgb_inst_1.destroy()
                break
